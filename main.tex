...
\caption*{$H(A)$}
\begin{algorithmic}
\For{$x\in$ all possible strings\footnote{If it wasn't clear: we're iterating in increasing length. This procedure is sometimes also jokingly referred to as the \textit{British Museum algorithm} because  it's akin to using chimps in front of typewriters in an attempt to reproduce all the books in the British Museum.}}
...
Consider the following algorithm:
\begin{algorithm}[H]
\caption*{$B(A)$\footnote{I picked the letters $P$ and $B$ because they visually somewhat resemble the programs they denote. (Also, this is why I placed the ``doesn't halt" case first.)}}
\begin{algorithmic}
\For{$x\in$ all possible strings}
  \If{$x$ proves <<$A(A)$ doesn't halt>>}
    \State halt
...
\impl We can write out this computation in $\mathcal{F}$ and point out that $B(B)$ will then find $p$ and enter an infinite loop.\footnote{Proving that a program runs forever raises the specter ($\mathghost$) of sub-inconsistency. But rest assured. Any formal system capable of reasoning about algorithms can prove that ``\textbf{while true} \{\}" runs forever. Otherwise it ought to be ashamed of itself.}
